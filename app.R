...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectizeInput("corr_x", "X Variable", choices = numeric_vars, selected = numeric_vars[1]),
      selectizeInput("corr_y", "Y Variable", choices = numeric_vars, selected = numeric_vars[2]),
      ...
      radioButtons("schl_corr", "Education attainment", 
                   choices = list("All" = "all", "High School not Completed" = "no_hs", "High School or GED" = "hs", "College Degree" = "college")),
      
      h2("Select a Sample Size"),
      sliderInput("corr_n", "Sample Size", min = 10, max = 500, value = 100),
    ...
    mainPanel(
      ...
      conditionalPanel("input.corr_sample",
                       ...
                       column(6, 
                              numericInput("corr_guess", "", value = 0, min = -1, max = 1)
                       ),
                       column(6, 
...
output$scatter_plot <- renderPlot({
  validate(need(!is.null(sample_corr$corr_data), "Please select your variables, subset, and click the 'Get a Sample!' button."))
  #this is a useful function to add as a placeholder until data is generated!
  ggplot(sample_corr$corr_data, aes_string(x = input$corr_x, y = input$corr_y)) + geom_point()
  })
